...
Original file is located at
    https://colab.research.google.com/drive/1CBVMKiSJ6CLnvzYkJq3G0fbcELF9x8C1
"""
...
class VQAModel(nn.Module):
    ...
    def forward(self, image, question_tokens):
        ...
        question_feature = self.text_encoder(**question_tokens).last_hidden_state[:, 0, :]
...
def train(model, dataloader, optimizer, criterion, device):
    ...
    for image, question_tokens, answers, mode_answer in dataloader:
        image, question_tokens, answers, mode_answer = image.to(device), {k: v.squeeze(1).to(device) for k, v in question_tokens.items()}, answers.to(device), mode_answer.to(device)
...
def main():
    ...
    train_dataset = VQADataset(df_path="/content/data/train.json", image_dir="/content/data/train", transform=transform, tokenizer=tokenizer)
    test_dataset = VQADataset(df_path="/content/data/valid.json", image_dir="/content/data/valid", transform=transform, answer=False, tokenizer=tokenizer)
    ...
    for image, question_tokens in test_loader:
        image, question_tokens = image.to(device), {k: v.squeeze(1).to(device) for k, v in question_tokens.items()}
        with torch.no_grad():
            pred = model(image, question_tokens)
